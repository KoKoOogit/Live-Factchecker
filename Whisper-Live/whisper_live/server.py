...
class ServeClientFasterWhisper(ServeClientBase):
    ...
    def __init__(self, websocket, task="transcribe", device=None, language=None, client_uid=None, model="small.en",
        ...
        self.gemini_api_key = "AIzaSyAwqHOXC_dV_1FN6usIXVC6wLjxD7PwV1Y"
        ...
        self.speaker_embedding_model = Inference("pyannote/embedding", use_auth_token="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")
        self.previous_embedding = None
        self.speaker_change_threshold = 0.3
    ...
    def save_audio_chunk_as_wav(self, input_sample, sample_rate=16000, file_path="audio_chunk.wav"):
        """
        Saves a NumPy audio chunk as a WAV file.
        
        Args:
            input_sample (np.array): The audio chunk to be saved.
            sample_rate (int): The sample rate of the audio.
            file_path (str): The file path to save the WAV file.
            
        Returns:
            str: The path to the saved WAV file.
        """
        sf.write(file_path, input_sample, sample_rate)
        return file_path

    
    def detect_speaker_change(self, embedding):
        try:
        ...
        try:
            self.previous_embedding = np.asarray(self.previous_embedding).flatten()
            distance = cosine(self.previous_embedding, embedding)
            if distance > self.speaker_change_threshold:
                self.previous_embedding = embedding
                return True
        except Exception as e:
    ...
    def extract_speaker_embedding(self, audio_chunk):

        try:

            wav_file = self.save_audio_chunk_as_wav(audio_chunk)
        except Exception as e:
            print(f"Something wrong{e}" )

        try:
            embedding = self.speaker_embedding_model(wav_file)
        except Exception as e:
            print("Sometihng wrong with this ")
        return embedding
    ...
    def send_to_gemini_api(self, text):
        """
        Sends the transcribed text to the Gemini API for fact-checking
        ...
        Please analyze the following text and detect any claim, opinion, statement, or fact.
        This is fed from a live transcript, so be mindful of random chunks or incomplete sentences.

        Text: {text}

        Note: some of the sentences are cut off, or partially generated, because you are getting input from live transcription.
        Please be wary of that.  Also, please keep all of your knowledge into the text, and only the text.

        Output format (follow this strictly):

            1. Read the text and label them correctly
            2. Provide an explanation for anything you fact checked
            3. Indicate when a detected fact, claim, opinion, or statement it TRUE, FALSE, or N/A and just display those words in capital
        ...
        try:
            ...
            generated_text = "Error in generating response"

        return generated_text

    def send_gemini_response_to_client(self, response):
        """
        Send the Gemini API response to the client
    ...
    def handle_transcription_output(self, result, duration, embedding):
        segments = []
        if len(result):
            self.t_start = None
            ...
            if self.detect_speaker_change(embedding):
                transcription_text = " ".join([segment["text"] for segment in segments])
                gemini_response = self.send_to_gemini_api(transcription_text)
                self.send_gemini_response_to_client(gemini_response)
        else:
    ...
    def speech_to_text(self):
        ...
        while True:
            ...
            try:
                ...
                embedding = self.extract_speaker_embedding(input_sample)
                print(embedding)
                ...
                if result is None or self.language is None:
                    ...
                    continue
                self.handle_transcription_output(result, duration, embedding)
